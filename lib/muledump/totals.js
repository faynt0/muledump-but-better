...
function init_totals() {
    ...
    $('#totals .list').find('div.item').each(function() {
    	$(this).remove();
	});
    totals = {};
    ids = [];
...
function update_totals() {
	...
	function isbad(id) {
	    ...
	    if (!options.famefilter && !options.feedfilter && !options.utfilter && !options.sbfilter && !options.stfilter && !options.nonsbfilter && !options.wbfilter && !options.forgefilter && !options.osancfilter && !options.oryxtokenfilter) return false;
        ...
        if ( options.forgefilter && i[10] !== 'legendary' ) return true;
        if ( options.osancfilter && i[10] !== 'o3' ) return true;
        if ( options.oryxtokenfilter && i[10] !== 'oryxtoken' ) return true;
		return false;
	...
	*/
	...
	var itemList= new List('totals', listOptions);
