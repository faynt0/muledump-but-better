...
// dom snippet generators
    ...
    function makeList(classname, items, showFilter){
        let $vaults = $('<div>').addClass(classname);
        $vaults.attr('id', classname);
        if (showFilter) {
            $('<input class="search" placeholder="Search" />' +
                ...
                'Name' +
                '</button>' +
                '<button class="sort" data-sort="fp" data-default-order="asc">\n' +
                'FP' +
                '</button>' +
                '<button class="sort" data-sort="filter">' +
                'Special' +
                '</button>').appendTo($vaults);
        }

        let $itemList = $('<div>').addClass('list');
        items.forEach(function (item){
            if ( typeof window.items[Number(item)] === 'undefined' ) {
                setuptools.tmp.constantsRemovedItems.push(Number(item));
                setuptools.config.disqualifiedItemIds.push(Number(item));
            }
            setuptools.app.muledump.item(item, undefined, undefined, [classname, i]).appendTo($itemList);
        });

        $itemList.appendTo($vaults);

        return $vaults;
    }

    function makeExaltationTable(exaltData,guid,muleName){
        var $d = $('<div>').addClass('exalts-container');
        var $fl = $('<div>').attr('id','exalt-lightbox-'+ muleName);
        $fl.addClass('mule-exalts');
        $fl.addClass('hidden');
        var button = (`<div class="button" data-featherlight="#exalt-lightbox-${muleName}"title="Show Exaltations">∈</div>`);
        var $t = $('<table cellspacing="10">').addClass('exalts-table');
        var $tb = $('<tbody>');
        $('[data-guid="'+ guid +'"]').find('.muleMenu').after(button);
        ...
        $t.append('<thead><tr><th>Class</th><th>HP</th><th>MP</th><th class="atk">Atk</th><th class="def">Def</th><th class="spd">Spd</th><th class="dex">Dex</th><th class="vit">Vit</th><th class="wis">Wis</th><th>Total missing</th></tr></thead>');
    ...
    function maketable(classname, items) {
        var $t = $('<div>').addClass(classname);
        for (var i = 0; i < items.length; i++) {
            $('<div class="cont">').append(items[i]).appendTo($t);
        }
        return $t;
    }
...
// Mule object
    ...
    Mule.prototype.query = function(ignore_cache, cache_only, freshness, MuleQueue) {
        ...
        window.realmAPI(
            'list.php',
            { guid: CR.guid, password: CR.password },
            { url: realmApiURL },
            function(xhr) {
        ...
        function onResponse(xml, textStatus, request) {
            ...
            if ( error = xml.match(/^<Error\/?>(.*?)<\/?Error>$/) ) {
                ...
                // Keep old object structure to avoid refactoring smielyface
                JSONData.Chars = {
                    Char: JSONData.Char,
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        var forgeFire = d.Account.ForgeFireEnergy;
        ...
        setuptools.data.accounts.accounts[this.guid].lastReload = (new Date).getDate() + "." + ((new Date).getMonth() + 1) + "." + (new Date).getFullYear();
        ...
        if ( this.loginOnly === false ) {
            ...
            $name.prop('title', setuptools.data.accounts.accounts[this.guid].lastReload);
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            if ( this.loginOnly === false && !this.opt('shrink') && this.opt('hpmp')) {
                ...
                var quickslots = c.EquipQS.split(',');
                quickslots.forEach((qsItem, index) => {
                    var qsItemArray = qsItem.split('|');
                    var qsItemData = (qsItemArray[0] !== 'None') ? items[qsItemArray[0]] : items['-1'];
                    if (Array.isArray(qsItemData)){
                        var qsName = qsItemData[0];
                        hpmp.append($('<div style="margin-bottom: 5px;" class="consumable con'+ index +'">' + qsName + ' : ' + qsItemArray[1] + '</div>'));
                    } else {
                        hpmp.append($('<div style="margin-bottom: 5px;" class="consumable con'+ index +'">' + 'Unknown' + '</div>'));
                    }
        ...
        if (this.opt('exalts') && q.results.PowerUpStats != null) {
            //console.log(q.results.PowerUpStats);
            this.dom.append($('<hr>'));
            makeExaltationTable(q.results.PowerUpStats,self.guid, d.Account.Name).appendTo(this.dom);
        }
        ...
        function makeItemList(chests) {
            let arr = [];
            chests.forEach(function (chest) {
                if (chest !== 0) {
                    arr = arr.concat(chest.split(","));
                }
            });
        ...
        if (this.opt('vaults')) {
            ...
            if ( this.loginOnly === false && !this.opt('shrink') ) this.dom.append($('<hr class="vaults-seperator">'));
            ...
            var spoils = d.Account.TemporaryGifts ? d.Account.TemporaryGifts : ['-1'];
            if (typeof spoils === 'string') spoils = [spoils];
            if ((typeof spoils === 'object' && Object.keys(spoils).length === 0) || typeof spoils == 'undefined') spoils = ['-1'];
            ...
            arr = makeItemList(chests);

            spoils = makeItemList(spoils);




            if ( this.loginOnly === false && !this.opt('shrink') ) makeList('vaults', arr,1).appendTo(this.dom);

            // Seasonal spoils
            if ( this.loginOnly === false && !this.opt('shrink') ) makeList('spoilchest', spoils,0).appendTo(this.dom);
            ...
            var options = {
                item: "<div class='item' data-itemid data-name data-fp data-filter><div></div></div>",
                valueNames: [
                    { data: ['itemid']},
                    { data: ['name']},
                    { data: ['fp']},
            ...
            var itemList= new List('vaults', options);
        }
        ...
        if ( Array.isArray(items) === true && this.totals.cache === false ) {
            for (var tg = 0; tg < items.length; tg++) {
                ...
                countCoupon(items[tg]);
            }
        }
        var spoils = d.Account.TemporaryGifts ? d.Account.TemporaryGifts : ['-1'];
        var spoilsItems = ( typeof spoils === 'string' ) ? spoils.split(',').reverse() : undefined;
        if ( Array.isArray(spoilsItems) === true && this.totals.cache === false ) {
            for (var tg = 0; tg < spoilsItems.length; tg++) {
                this.totals.count(spoilsItems[tg], 'gifts');
                countCoupon(spoilsItems[tg]);
                console.log(spoilsItems[tg]);
            }
        }
        ...
        // display forge fire
        const maxForgeFire = 1600;
        if ( d.Account.Name === "Fyt") {
            let $forgeFire = $('<div>').addClass('forgefire');
            $forgeFire.text(forgeFire + " / " + maxForgeFire);
            $forgeFire.insertAfter(this.dom.find('.scont'));

            let $lastReload = $('<div>').addClass('lastreload');
            $lastReload.text(setuptools.data.accounts.accounts[this.guid].lastReload);
            $lastReload.insertAfter(this.dom.find('.forgefire'));
        }
        ...
        // display coupons
        if (charCoupon) {
            var $charCoupon = $('<div>').addClass('charcoupon');
            var $charCouponText = $('<div>').text(charCoupon);
            $charCouponText.appendTo($charCoupon);
            $charCoupon.insertAfter(this.dom.find('.scont'));
        }

        if (vaultCoupon) {
            var $vaultCoupon = $('<div>').addClass('vaultcoupon');
            var $vaultCouponText = $('<div>').text(vaultCoupon);
            $vaultCouponText.appendTo($vaultCoupon);
            $vaultCoupon.insertAfter(this.dom.find('.scont'));
        }
