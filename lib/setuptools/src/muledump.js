...
setuptools.app.muledump.drawPortrait = function(c, $c) {
    ...
    var chtext = $('<div>');
    var seasonal = c.Seasonal == "True" ? "seasonal" : "";
    if ( typeof boost !== 'undefined' ) chtext.append(boost);
    chtext.append($('<div style="padding-top: 3px;">').text('<span class="' + seasonal + '">' + cl[0] + '</span> ' + c.Level + ", " + c.muledump.MaxedStats + "/8" + ', #' + c.id)).append($('<div>').text(c.CurrentFame + ' F ' + c.Exp + ' XP'));
...
setuptools.app.muledump.item = function(id, type, num, ext) {
    ...
    $r.attr('data-name', it[0]);
    $r.attr('data-fp', it[6]);
    $r.attr('data-filter', it[10]);
