...
setuptools.app.muledump.totals.menu.main = function(track) {
    ...
    if ( setuptools.data.options.totals === true ) {
        ...
        options.push({
            ...
            {
                class: 'toggleForgeFilter',
                name: ( (setuptools.data.options.forgefilter === true) ? 'Disable' : 'Show Only' ) + ' Legendary Forge',
                callback: setuptools.app.muledump.toggleoption,
                callbackArg: 'forgefilter'
            },
            {
                class: 'toggleOsancFilter',
                name: ( (setuptools.data.options.osancfilter === true) ? 'Disable' : 'Show Only' ) + ' Oryx 3 Tiered',
                callback: setuptools.app.muledump.toggleoption,
                callbackArg: 'osancfilter'
            },
            {
                class: 'toggleOryxtokenFilter',
                name: ( (setuptools.data.options.oryxtokenfilter === true) ? 'Disable' : 'Show Only' ) + ' Oryx Token Gear',
                callback: setuptools.app.muledump.toggleoption,
                callbackArg: 'oryxtokenfilter'
            }
        );
...
setuptools.app.muledump.itemsSlotTypeMapper = function(type) {
    ...
    var mapKeys = {
        ...
        soulbound: 8,
        utst: 9,
        label: 10 // label wie legedary forge / o3 tiered
    };
...
setuptools.app.muledump.totals.forgeItems = function(config) {
    ...
    setuptools.lightbox.settitle('main', 'Forge Items');
